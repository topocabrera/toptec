...
function EditClients() {
    ...
    useEffect(() => {
        ClientsDataService.getSeguros()
            .orderByChild('id')
            .equalTo(parseInt(state.id, 10))
            .once('value', onDataChange);
    }, []);

    const onDataChange = (items) => {
        let key = Object.keys(items.val());
        let data = Object.values(items.val());
        ...
        data?.forEach(client => {
            setState(prevState => ({
                ...
                dni: client?.dni,
                email: client?.email,
                fechaNac: moment(client?.fechaNac, "DD/MM/YYYY").toDate(),
            ...
            client?.vehiculos && setVehiculos(client?.vehiculos)
    })

        // setState(prevState => ({ ...currentMember }));
    }
    ...
    const saveClient = () => {
        ...
        ClientsDataService.updateClientSeguro(state?.key, data)
            .then(() => {
                setState({
                    submitted: true,
                });
